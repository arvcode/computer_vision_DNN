...
class robot:
    ...
    def sense(self):
        ...
        # Get the distance. Do not take absoulte value.
            dx=landmark_x-self.x +self.rand()*self.measurement_noise
            dy=landmark_y-self.y +self.rand()*self.measurement_noise
        
        #Check if dx,dy are within range
            if (dx>self.measurement_range): #or (dx<0.0):
                continue
            if (dy>self.measurement_range): #or (dy<0.0):
                continue
